...
export default class ExtensionEventChannel {
  ...
  static setIsLoading(isLoading: boolean, message?: string): void {
    ...
    document.dispatchEvent(
      new CustomEvent<LoadingEventDetail>(this.LOADING_EVENT_NAME, { detail })
    );
  }
  ...
  static onLoadingUpdate(
    handler: (detail: LoadingEventDetail) => void
  ): () => void {
    ...
    return () =>
      document.removeEventListener(this.LOADING_EVENT_NAME, listener);
  }
}
