...
export default class Section {
  ...
  static getSectionFromJSON(data: any): Section {
    const sectionDetails = data.sectionDetails.map(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (sectionDetail: any) =>
        SectionDetail.getSectionDetailFromJSON(sectionDetail)
    );
  ...
  getLocations(term?: number[]): string[] {
    ...
    this.sectionDetails.forEach((sectionDetail: SectionDetail) => {
      if (
        term &&
        !sectionDetail.getTerms().some((t: number) => term.includes(t))
      )
        return;
  ...
  getTerms(): Set<number> {
    return this.sectionDetails.reduce(
      (acc: Set<number>, sectionDetail: SectionDetail) => {
      ...
      new Map(
        this.sectionDetails
          ...
          .map((sectionDetail: SectionDetail) => {
            ...
            const key = `${item.day.sort().join(",")}|${item.startTime}|${
              item.endTime
            }|${item.terms?.sort().join(",")}`;
            return [key, item];
          })
      ).values()
    );
  ...
  exportToJSON(): any {
    return {
      ...
      sectionDetails: this.sectionDetails.map((sectionDetail: SectionDetail) =>
        sectionDetail.exportToJSON()
      ),
