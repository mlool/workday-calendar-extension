...
async function handleCopyScheduleButtonClick(
  ...
  if (!tables.length) {
    alert(
      "Failed to find Course Schedule Table, if you believe this is an error please contact developers through Discord."
    );
    return;
  }
  ...
  if (counter < 0 || counter >= tables.length) {
    alert(
      `Button does not seem to be associated with a valid table. Valid range should be between 0 to ${
        tables.length - 1
      }, please contact developers through Discord.`
    );
    return;
  }
  ...
  try {
    ...
    if (selectedTerms.size === 1) {
      await ExtensionStorage.setCurrentTerm(
        selectedTerms.values().next().value ?? 1
      );
    } else {
    ...
    alert(
      "An error has occured while copying the schedule, you may try an alernative method of adding the sections individually and contact developers through discord."
    );
  } finally {
