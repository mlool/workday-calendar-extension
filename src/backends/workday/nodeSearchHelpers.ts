...
export function collectNodesWithLabel(
  root: WorkdayNode[] | WorkdayNode
): WorkdayNode[] {
...
export function extractWorkdaySectionInfo(selectedNodes: WorkdayNode[]) {
  ...
  const instructorNode = selectedNodes.find(
    (node) => node.label === "Instructor Teaching"
  );
  const instructionalFormatsNode = selectedNodes.find(
    (node) => node.label === "Instructional Formats"
  );
  const meetingPatternsNode = selectedNodes.find(
    (node) => node.label === "Meeting Patterns"
  );

  const rawName = courseNode?.instances?.[0]?.text;
  const instructors =
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    instructorNode?.instances?.map((instance: any) => instance.text) || [];
  const format = instructionalFormatsNode?.instances?.[0]?.text;
  const meetingPatterns =
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    meetingPatternsNode?.instances?.map((instance: any) => instance.text) || [];
...
export const parseSessionAndTermFromDateRange = (
  ...
  return {
    session: finalSessions.values().next().value!,
    terms: Array.from(finalTerms),
  };
};
