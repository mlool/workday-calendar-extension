...
import {
  collectNodesWithLabel,
  convertTo24HourFormat,
  extractWorkdaySectionInfo,
  parseSessionAndTermFromDateRange,
} from "./nodeSearchHelpers";
...
export function fetchSectionFromJSON(
  ...
  const selectedNodes = collectNodesWithLabel(
    data["body"]["children"][0]["children"]
  );

  const { code, name, instructors, format, meetingPatterns } =
    extractWorkdaySectionInfo(selectedNodes);
...
function getSectionDetailFromMeetingPattern(meetingPattern: string): {
  session: string;
  sectionDetail: SectionDetail;
} {
  const tokens = meetingPattern
    .split("|")
    .map((t) => t.trim())
    .filter(Boolean);
  ...
  const [startTime, endTime] =
    result.timeRange?.split(" - ").map(convertTo24HourFormat) ?? [];
  const [startDate, endDate] = result.dateRange?.split(" - ") ?? [];
  const { session, terms } = parseSessionAndTermFromDateRange(
    result.dateRange ?? ""
  );
  const days =
    result.daysString?.split(" ").map((day: string) => day.trim()) ?? [];

  const building = buildingBits.join(" ");
  const location =
    building || result.floor || result.room
...
export async function extractSection(element: Element) {
  ...
  if (!courseId) {
    throw new Error(
      "Course ID not found, please manually add the section by url"
    );
  }
