...
const SectionDetails = ({ section, term, onClose, onDelete }: IProps) => {
  return (
    <div className="section-details-overlay" onClick={onClose}>
      <div
        className="section-details-popup"
      ...
      >
        <div className="popup-header">
          <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            ...
            {section.getSectionLink() !== "" && (
              <ExternalLinkIcon size={16} href={section.getSectionLink()} />
            )}
          </div>
        ...
        <div className="popup-content">
          ...
          {!section.getIsCustom() && (
            <LocationComponent section={section} term={term} />
          )}
        </div>
...
const InstructorComponent = ({ section }: { section: Section }) => {
  const [rmpRatings, setRmpRatings] =
    ...
    return (
      <div className="detail-link">
        ...
        <ExternalLinkIcon
          size={14}
          href={
            rmpRatings?.[instructor]?.link ??
            `https://www.google.com/search?q=${instructor + " UBC"}`
          }
        />
      </div>
    );
        ...
        section.getInstructors().map((instructor, index) => (
          <div className="detail-row" key={index}>
            ...
            <Rating
              ...
              rating={rmpRatings?.[instructor]?.rating}
            />
          </div>
        ))
      ) : (
      ...
      {grades ? (
        <>
          <div className="detail-row">
            ...
            <div className="detail-link">
              {grades.average ? grades.average.toFixed(2) + "%" : "N/A"}{" "}
              <ExternalLinkIcon size={14} href={section.getGradesUrl()} />
            </div>
          </div>
          <div className="detail-row">
            <span className="detail-value">Average (5 Years)</span>
            <div className="detail-link">
              {grades.averageFiveYears
                ? grades.averageFiveYears.toFixed(2) + "%"
                : "N/A"}{" "}
              <ExternalLinkIcon size={14} href={section.getGradesUrl()} />
            </div>
          </div>
    ...
    <div className="detail-section">
      ...
      {locations.length > 0 ? (
        locations.map((loc, index) => (
          <div className="detail-row" key={index}>
            <span className="detail-value" style={{ flex: 2 }}>
              {loc}
            </span>
          </div>
        ))
