...
const CustomSectionDetails = ({
  ...
  useEffect(() => {
    if (section) {
      ...
      const inSchedule = schedule
        .getSections()
        .some((s: Section) => s.getCourseID() === section.getCourseID());
  ...
  const handleSave = async () => {
    ...
    const detail = new SectionDetail(termsArray, daysArray, startTime, endTime);
    ...
    if (section && oldDetail) {
      ...
      const daysChanged =
        oldDays.size !== selectedDays.size ||
        !daysArray.every((d) => oldDays.has(d));

      const oldTerms = section.getTerms();
      const termsChanged =
        oldTerms.size !== selectedTerms.size ||
        !termsArray.every((t) => oldTerms.has(t));

      if (
        !daysChanged &&
        !termsChanged &&
        oldDetail.getStartTime() === startTime &&
        oldDetail.getEndTime() === endTime
      ) {
    ...
    if (scheduleChanged) {
      ...
      if (isExisting && section) {
        const newSchedule = schedule.removeSection(
      ...
      if (isExisting && section) {
        let newSchedule = schedule.removeSection(
          section.getWorklistNumber(),
          section.getCourseID()
        );
  ...
  return (
    <div className="custom-section-details-overlay" onClick={onClose}>
      <div
        className="custom-section-details-popup"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="popup-header">
          <h2 className="popup-title">
            {section ? "Edit Custom Section" : "New Custom Section"}
          </h2>
