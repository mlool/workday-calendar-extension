...
const DownloadICS = ({
  ...
  const onClick = () => {
    ...
    schedule.getSections().forEach((section) => {
      ...
      section.getSectionDetails().forEach((detail) => {
        ...
        const recurrenceRule = `FREQ=WEEKLY;BYDAY=${days
          .map(getDayMap)
          .join(",")};UNTIL=${formatDate(
        ...
        icsContent += `DESCRIPTION:${section.getName()} - ${section
          .getInstructors()
          .join(", ")}\n`;
  ...
  return <DownloadICSIcon disabled={disabled} size={22} onClick={onClick} />;
};
