...
describe("Section tests", () => {
  ...
  beforeEach(() => {
    sectionDetail = SectionDetail.getSectionDetailFromJSON(
  ...
  test("getting terms offered and testing chrome storage interactions", () => {
    ...
    expect(section.getSectionLink()).toStrictEqual(
      "https://wd10.myworkday.com/ubc/d/inst/1$15194/15194$458199.htmld"
    );
  });
  ...
  test("getLocations returns unique locations only", () => {
    const sectionWithDupDetails = new Section(
      "DSCI_V 200",
      "458199",
      [],
  ...
  test("historical grades test for full coverage", async () => {
    global.fetch = jest.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ average: "", average_past_5_yrs: "" }),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any);
  ...
  test("get grades url test", () => {
    const sectionTest2 = Section.getSectionFromJSON(sectionData.data[1]);
    expect(sectionTest2.getGradesUrl()).toStrictEqual(
      "https://ubcgrades.com/statistics-by-course#UBCV-CPSC-213"
    );
  });
  ...
  test("get section link", () => {
    const sectionTest2 = Section.getSectionFromJSON(sectionData.data[1]);
    expect(sectionTest2.getSectionLink()).toEqual(
      "https://wd10.myworkday.com/ubc/d/inst/1$15194/15194$14847.htmld"
    );
    const sectionCustom = new Section(
      "DSCI_V 200",
      "458199",
  ...
  test("testing get locations", () => {
    ...
    expect(term2Locations).toEqual(["Brock Commons South (BRCS) - Room 2070"]);
