...
beforeEach(() => {
  ...
  sectionDetail2 = SectionDetail.getSectionDetailFromJSON(
...
describe("Schedule Tests", () => {
  ...
  test("bulk add sections conflicting branch", async () => {
    ...
    jest
      .spyOn(ExtensionStorage, "getIsConflictAddingEnabled")
      .mockResolvedValue(false);
    const schedule = new Schedule(Schedule.getVersion(), []);
    const scheduleWithSection = await schedule.addSection(section213);
    const result = await scheduleWithSection.bulkAddSections([
      sectionDSCI,
      section221,
    ]);
  ...
  test("remove sections", async () => {
    ...
    expect(scheduleAdded.getSections()).toStrictEqual([
      sectionDSCI,
      section221,
    ]);
  ...
  test("get conflict sections when sections overlap", async () => {
    ...
    expect(scheduleWithOne.getConflictSections(section200)).toEqual([
      section121,
    ]);
  });
  ...
  test("get conflicts with multiple conflicts", async () => {
    ...
    const scheduleWithTwo = await (
      await schedule.addSection(cpsc121)
    ).addSection(dsci200);
  ...
  test("get colors", async () => {
    ...
    const detailForColors = SectionDetail.getSectionDetailFromJSON(
      json1.data[0].sectionDetails[0]
    );
    ...
    schedule = new Schedule(
      Schedule.getVersion(),
      [section1, section2, sectionOtherWorklist, sectionOtherSession],
      id
    );
  ...
  test("get course color", async () => {
    ...
    expect(schedule.getCourseColor("2025W", 0, "CPSC_V 110")).toBe(firstColor);

    const detailForCourseColor = SectionDetail.getSectionDetailFromJSON(
      json1.data[0].sectionDetails[0]
    );
    ...
    expect(schedule.getCourseColor("2025W", 0, "CPSC_V 121")).toBe(firstColor);

    const newColor = schedule.getCourseColor("2025W", 0, "DSCI_V 200");

    expect(newColor).not.toBe(firstColor);
    expect(SECTION_COLORS).toContain(newColor);

    const otherSessionColor = schedule.getCourseColor("2024W", 1, "CPSC_V 110");
  ...
  test("getScheduleFromExternalJSON replaces all worklists in a session", async () => {
    ...
    expect(result.getSections().some((s) => s.getSession() === "2024W")).toBe(
      true
    );
  });
  ...
  test("getScheduleFromExternalJSON replaces only specified worklist in session", async () => {
    ...
    expect(result.getSections().length).toBe(2);
    expect(result.getSections().some((s) => s.getWorklistNumber() === 0)).toBe(
      true
    );
  });
