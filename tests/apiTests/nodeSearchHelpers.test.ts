...
test("Load ACAM_V 100 001 2025W", async () => {
    ...
    expect(detailsOne?.getLocation()).toBe('Buchanan Building (BUCH) | Floor: 2 | Room: D222');
    ...
    expect(detailsTwo?.getLocation()).toBe('Buchanan Building (BUCH) | Floor: 2 | Room: D222');
...
test("Load BIOL_V 155 001 2025W", async () => {
    ...
    expect(section?.getSectionCode()).toBe("001");
    expect(section?.getName()).toBe('Human Biology: Physiology and Introductory Anatomy');
    ...
    expect(section?.getFormat()).toBe("Lecture");
    expect(section?.getInstructors()).toStrictEqual(['Irene Ballagh', 'Queenie Hui']);
    expect(section?.getIsCustom()).toBe(false);
    ...
    expect(detailsOne?.getLocation()).toBe('Gordon B. Shrum Building (SHRM) | Floor: -1 | Room: B1001');
    ...
    expect(detailsTwo?.getLocation()).toBe('Pharmaceutical Sciences Building (PHRM) | Floor: 1 | Room: 1101');
    expect(detailsTwo?.getTerms()).toStrictEqual([2]);
    ...
    expect(detailsThree?.getLocation()).toBe('Pharmaceutical Sciences Building (PHRM) | Floor: 1 | Room: 1101');
