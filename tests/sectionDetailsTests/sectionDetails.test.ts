...
describe("Section Details Tests", () => {
  ...
  test("constructor sets variables correct and also the optional variables with getters tests", () => {
    const sectionDetail = new SectionDetail(
      [1, 2],
      ["Mon", "Wed", "Fri"],
      "03:30",
    ...
    expect(sectionDetail.getLocation()).toBe(
      "Biological Sciences Building (BIOL) | Floor: 1 | Room: 1000"
    );
  });
  ...
  test("export to json without optional", () => {
    const sectionDetail = new SectionDetail([1], ["Tue"], "10:00", "12:00");
